package com.ggz.baselibrary.retrofit.down;
...
public class DownManager {
                ...
                .flatMap(new Function<ResponseBody, ObservableSource<Object>>() {
                    ...
                    public ObservableSource<Object> apply(ResponseBody body) throws Exception {
                        ...
                        for (int i = 0; i < THREAD_COUNT; i++) {
                            //开始位置，获取上次取消下载的进度
                            long startPosition = mCache.getAsLong(url + i + ConstantUtils.DownTherad);
                            //如果文件已经删除 则重新下载
                            if (targetFile.length() == 0) {
                                startPosition = 0L;
                                info.getReadLength().set(0L);
                                //删除 每个线程 已经下载的 总长度 缓存
                                mCache.remove(url + i + ConstantUtils.DownTherad);
                            }
    ...
    private void writeCaches(@NonNull String nameKey, @NonNull final long start,
        ...
        try {
            ...
            ACache mCache = ACache.get(ConfigUtils.getAppCtx());
            //当前线程下载 总长度
            long loaded = mCache.getAsLong(nameKey + ConstantUtils.DownTherad);
            ...
            while ((len = is.read(buffer)) != -1) {
                ...
                //缓存当前线程 下载内容 总长度
                mCache.put(nameKey + ConstantUtils.DownTherad, loaded);
            }
        } finally {
            if (null != is) {
                is.close();
    ...
    public void addDownTask(DownInfo downInfo, DownLoadListener loadListener) {
        ...
        //获取缓存中 所有的下载任务列表
        ACache mCache = ACache.get(ConfigUtils.getAppCtx());
        String jsonArray = mCache.getAsString(ConstantUtils.AllDownTask);
    ...
    public void saveDownInfo(DownInfo downInfo) {
        ...
        mCache.put(ConstantUtils.AllDownTask, GsonUtils.listToJson(list));
    }
