...
public class IdCardUtil {
    ...
    public static String hideName(String name) {
        char[] oldchar = name.toCharArray();
        if (oldchar.length == 2) {
            oldchar[1] = '*';
            return String.valueOf(oldchar);
        } else if (oldchar.length >= 3) {
            return oldchar[0] + "*" + oldchar[oldchar.length - 1];
        }
        return name;
    }

    public static String hideIdcard(String idcard) {
        char[] oldIdcard = idcard.toCharArray();
        for (int i = 6; i < 12; i++) {
            oldIdcard[i] = '*';
        }
        return String.valueOf(oldIdcard);
    }
}
